...
const masterNavigation = [
  ...
  { name: 'Holidays', href: '/master/holidays', icon: CalendarDays },
];
    ...
    <>
      ...
      {isMobileOpen && (
        <div
          ...
          className="fixed inset-0 bg-black/60 backdrop-blur-xs z-40 md:hidden animate-fade-in transition-opacity"
          aria-hidden="true"
        />
      )}
      ...
      <aside
        className={cn(
          "flex flex-col fixed left-0 top-0 h-full w-64 bg-surface-container border-r border-outline-variant py-6 px-4 gap-2 z-50 transition-transform duration-300 ease-in-out select-none shadow-xl md:shadow-none",
          // On desktop (md and up): static/visible in place
      ...
      >
        ...
        <div className="flex items-center justify-between px-2 mb-6">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-primary-container flex items-center justify-center shrink-0 shadow-xs">
          ...
          <button
            type="button"
            onClick={onCloseMobile}
            className="md:hidden p-1.5 text-secondary hover:text-on-background hover:bg-surface-container-high rounded-lg transition-colors cursor-pointer"
        ...
        {/* Navigation List */}
        <div className="flex flex-col gap-1 w-full flex-1 overflow-y-auto pr-1 custom-scrollbar">
          {/* Dashboard link */}
          <NavLink
            to="/"
          ...
          <div>
            ...
            <div
              className={cn(
                ...
                isMasterOpen ? "max-h-[500px] opacity-100 py-1" : "max-h-0 opacity-0 pointer-events-none"
              )}
            >
